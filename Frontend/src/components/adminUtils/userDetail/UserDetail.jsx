      ...
      <div className={style.divSales}>
        <h3>
          {userPurchases?.length
            ? `Ventas: ${userPurchases?.length}`
            : "No hay ventas"}
        </h3>
        <div className={style.tableGrid}>
          {userPurchases?.map((purchase, index) => (
            <div
              key={`${purchase.idProduct}-${index}`}
              className={style.purchaseCard}
            >
              <table className={style.purchaseTable}>
                <thead>
                  ...
                  <tr>
                    <th>Producto</th>
                    <th>Cantidad</th>
                    <th className={style.thPrice}>Precio</th>
                    <th>Total</th>
                    <th>Fecha</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    ...
                    <td className={style.thPrice}>
                      ${purchase.products[0].priceByProducts}
                    </td>
                    <td rowSpan={purchase.products.length}>
                      $
                      {purchase.products.reduce(
                        (total, product) => total + product.priceByProducts,
                        0
                      )}
                    </td>
                    <td rowSpan={purchase.products.length}>
